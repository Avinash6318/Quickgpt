  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-linear-to-b
     ...
     backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen&&'max-md:-translate-x-full'}`}>
      ...
      bg-linear-to-r from-[#A456F7] to-[#3D81F6] text-sm rounded-md cursor-pointer" onClick={()=>{navigate('/')}}>
      ...
      {/* Recent Chats */}
      {chats.length >0 && <p className='mt-4 text-sm'>
        Recent chats</p>}
      ...
      <div className="flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md 
       cursor-pointer group transition-all" >
        <img src={assets.user_icon} alt="" className="w-7 not-dark:invert rounded-full" />
        <p className="flex-1 text-sm dark:text-primary turncate">
      ...
      src={assets.close_icon} alt="" className="absolute top-3 right-3 w-5 h-5 cursor-pointer md:hidden not-dark:invert" />
